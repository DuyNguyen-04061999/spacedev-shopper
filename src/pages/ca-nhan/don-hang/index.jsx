...
export default function Order() {
  ...
  const { data: { count: pendingCount } } = useQuery({
    queryFn: () => orderService.count('?status=pending'),
  })
  const { data: { count: confirmCount } } = useQuery({
    queryFn: () => orderService.count('?status=confirm')
  })
  const { data: { count: shippingCount } } = useQuery({
    queryFn: () => orderService.count('?status=shipping')
  })
  ...
  return (
    <>
      ...
      <Tab disabledRemove name="order-tab" defaultActive="all"
